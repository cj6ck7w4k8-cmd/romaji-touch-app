...
function StatusMarks({ status, showEmpty = false }: { status: ReturnType<typeof stageStatus>; showEmpty?: boolean }) {
  ...
  return <span className="status-marks">{status.completed ? '○' : ''}{status.noMiss ? '★' : showEmpty ? '☆' : ''}{status.speed ? '⚡' : ''}{status.bestTime !== undefined && <small>{status.bestTime.toFixed(1)}びょう</small>}</span>
}
...
function Result({ choice, result, onRetry, onSelect, onRecord }: { choice: Choice; result: GameResult; onRetry: () => void; onSelect: () => void; onRecord: () => void }) {
  ...
  return <section className="result-page"><div className="result-card"><p className="eyebrow">よく できました！</p><div className="result-mark" aria-hidden="true">{choice.mode === 'practice' ? '☺' : clean ? '★' : '○'}</div><h2>{questionCount(choice)}もん できたよ！</h2>{choice.mode === 'practice' ? <p className="result-copy">れんしゅうを おわったよ。<br />つぎは ほんばんにも ちょうせんしてみよう！</p> : <><p className="result-copy">{clean ? 'まちがえずに できたね！' : 'さいごまで がんばったね！'}</p>{stars.length ? <><p className="earned-label">こんかい もらえた バッジ</p><div className="earned-stars">{clean && <span className="earned">★<small>まちがえ なし</small></span>}{quick && <span className="earned">⚡<small>はやく できた</small></span>}</div></> : <p className="earned-label no-badge">こんかいは バッジ なし。つぎも がんばろう！</p>}<p className="result-note">{choice.level === 3 ? `こんかいは ${result.seconds.toFixed(1)} びょう だったよ` : '⚡は 1もじ 2びょうより はやいと もらえるよ'}</p></>}<div className="result-actions">{choice.mode === 'practice' ? <button className="primary-button" onClick={onSelect}>ほんばんを えらぶ</button> : <div className="result-primary-actions"><button className="primary-button" onClick={onRetry}>もう いちど</button><button className="next-button" onClick={onSelect}>つぎの もんだいを えらぶ →</button></div>}<button className="result-record" onClick={onRecord}>きろくを みる →</button></div></div></section>
}
...
function recordLabel(key: string) { const [level, set, variant] = key.split('-'); const range = set === 'all' ? '' : ` ${set.split('').join('・')}`; return level === '3' ? `3 ${variant === 'random' ? 'ランダム' : 'じゅんばん'}` : `${level}${range}` }
function Record({ progress, onBack, reset }: { progress: Progress; onBack: () => void; reset: () => void }) { const entries = Object.entries(progress.stars); return <section className="content-page record-page"><button className="back-button" onClick={onBack}>← もどる</button><div className="page-heading record-heading"><p className="eyebrow">ここまでの あゆみ</p><h2>きろく</h2><p>できた もんだいを みてみよう。</p></div><div className="record-summary"><strong>{progress.completed.length}</strong><span>ステージ クリア</span></div>{entries.length === 0 ? <p className="empty-state">まだ きろくが ないよ。<br />まずは ひとつ あそんでみよう！</p> : <div className="records">{entries.map(([key, stars]) => <div className="record-row" key={key}><span>{recordLabel(key)}</span><div className="record-result"><b>{stars.includes('no-miss') ? '★' : '☆'} {stars.includes('speed') ? '⚡' : ''}</b>{progress.bestTimes[key] !== undefined && <small>{progress.bestTimes[key].toFixed(1)} びょう</small>}</div></div>)}</div>}<button className="reset-button" onClick={reset}>きろくを リセット</button></section> }
