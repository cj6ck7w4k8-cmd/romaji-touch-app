...
function Medals({ progress, onBack }: { progress: Progress; onBack: () => void }) {
  ...
  return <section className="content-page medals-page"><button className="back-button" onClick={onBack}>← もどる</button><div className="page-heading medal-heading"><h2>メダル</h2><p>{earned.length} / 10　ゲット</p></div><p className="medal-help">メダルを おすと ゲットの しかたが みられるよ。</p><div className="medal-grid">{medalDefinitions.map(medal => { const isEarned = earned.includes(medal.id); return <button key={medal.id} className={`medal-card ${isEarned ? 'earned' : ''}`} onClick={() => setDetail(medal.id)}><i aria-hidden="true">{isEarned ? medal.icon : ''}</i><span>{medal.title}</span><small>{medal.subtitle}</small></button> })}</div>{detail && <MedalPopup medals={[detail]} detail onClose={() => setDetail(null)} />}</section>
}
