...
export function App() {
  ...
  const select = (level: Level, set: string, random = false) => setChoice({ level, set, random, mode: 'challenge' })
  ...
  return <main className="app"><header><button className="logo" onClick={() => setView('home')}>ろーまじ<br/><b>ますたー</b></button><nav><button onClick={() => setView('book')}>ずかん</button><button onClick={() => setView('record')}>きろく</button></nav></header>
    ...
    {view === 'select' && <section><h2>れべるを えらぼう</h2><div className="level-grid"><LevelCard level={1} text={labels[1]} open onClick={() => select(1, groupNames[0])}/><LevelCard level={2} text={labels[2]} open={level2Open} onClick={() => select(2, pairs[0])}/><LevelCard level={3} text={labels[3]} open={level3Open} onClick={() => select(3, 'all')}/></div>
      <div className="sets"><h3>れべる1　1つ えらぼう</h3><div>{groupNames.map(g => <button key={g} onClick={() => select(1,g)}>{g}ぎょう</button>)}</div><h3>れべる2　2つ えらぼう</h3><div>{pairs.map(p => <button disabled={!level2Open} key={p} onClick={() => select(2,p)}>{p}ぎょう</button>)}</div><h3>れべる3</h3><button disabled={!level3Open} onClick={() => select(3,'all')}>じゅんばん</button><button disabled={!randomOpen} onClick={() => select(3,'all',true)}>らんだむ</button></div></section>}
    {view === 'book' && <Book />}
...
function Book(){return <section><h2>ろーまじ ずかん</h2><p>もじを たっぷして みてみよう</p><div className="book">{groupNames.map(g=><div key={g}><h3>{g}ぎょう</h3>{kanaGroups[g].map(x=><button key={x.kana}><b>{x.kana}</b><span>{x.roma}</span></button>)}</div>)}</div></section>}
