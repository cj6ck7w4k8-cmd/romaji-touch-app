...
function Home({ progress, currentLevel, onStart, onRecord, onMedals }: { progress: Progress; currentLevel: Level; onStart: (next: Choice) => void; onRecord: () => void; onMedals: () => void }) {
  ...
  return <section className="home-page"><div className="home-copy"><h1>ローマじを<br /><span>おぼえよう！</span></h1><p className="lead">ひらがなを みて、ローマじを えらぼう。</p><p className="next-stage">つぎは　<strong>{nextLabel}</strong></p><button className="primary-button primary-action" onClick={() => onStart(next)}>はじめる <span>→</span></button><div className="home-status"><button className="home-record" onClick={onRecord}><span className="home-record-title">いまの レベル</span><span className="home-record-stats"><strong>レベル{currentLevel}</strong></span><span className="home-record-help">きろくを みる →</span></button><button className="home-medal" onClick={onMedals}><span>メダル</span><strong>{medalCount} / 10</strong><small>みる →</small></button></div></div><div className="home-art romaji-lesson-art" aria-hidden="true"><div className="lesson-title">ひらがな　→　ローマじ</div><div className="lesson-card lesson-example"><b>あ</b><span>→</span><strong>a</strong></div></div></section>
}
