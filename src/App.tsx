...
function Game({ choice, back, finish }: { choice: Choice; back: () => void; finish: (result: { misses: number; seconds: number }) => void }) {
  ...
  const [index, setIndex] = useState(0), [misses, setMisses] = useState(0), [combo, setCombo] = useState(0), [hint, setHint] = useState(false), [hintUsed, setHintUsed] = useState(false), [started] = useState(Date.now()), [wrong, setWrong] = useState<string | null>(null), [mode, setMode] = useState<Mode>(choice.mode)
  ...
  const useHint = () => { if (!hintUsed) { setHintUsed(true); setMisses(m => m + 1) }; setHint(true) }
  const answer = (roma: string) => { if (roma === q.roma) { const next = index + 1; setCombo(c => c + 1); setWrong(null); if (next === questions.length) finish({ misses, seconds: (Date.now() - started) / 1000 }); else { setIndex(next); setHint(false); setHintUsed(false) } } else { setMisses(m => m + 1); setCombo(0); setWrong(roma); setHint(true) } }
  return <section className="game-page"><div className="game-top"><button className="back-button" onClick={back}>← もどる</button><span className="game-count">{index + 1} / {questions.length}</span><span className="combo-count">{combo > 1 ? `★ ${combo}` : ''}</span></div><div className="progress-track"><i style={{ width: `${(index / questions.length) * 100}%` }} /></div><div className="game-card"><div className="mode-switch"><button className={mode === 'practice' ? 'selected' : ''} onClick={() => setMode('practice')}>れんしゅう</button><button className={mode === 'challenge' ? 'selected' : ''} onClick={() => setMode('challenge')}>ほんばん</button></div><p className="prompt">この もじの ろーまじは？</p><div className="question-kana">{q.kana}</div><button className="in-game-hint" onClick={useHint}>{hint ? 'こたえを みたよ' : 'ひんとを みる'}</button>{hint && <p className="hint-text">こたえは <b>{q.roma}</b> だよ</p>}<div className={`answer-grid ${choice.level === 2 ? 'many' : ''}`}>{options.map(roma => <button key={roma} className={hint && roma === q.roma ? 'answer right' : wrong === roma ? 'answer wrong' : 'answer'} onClick={() => answer(roma)}>{roma}</button>)}</div></div></section>
}
