...
export function App() {
  ...
  return <main className="app-shell">
    ...
    {view === 'record' && <Record progress={progress} initialLevel={currentLevel} onBack={() => nav('home')} reset={() => { if (confirm('きろくを ぜんぶ けしますか？')) { clearProgress(); setProgress(loadProgress()) } }} />}
...
function Select({ progress, initialLevel, initialChoice, level2Open, level3Open, level4Open, level5Open, level6Open, level7Open, level8Open, randomOpen, onBack, onSelect }: { progress: Progress; initialLevel: Level; initialChoice: Choice | null; level2Open: boolean; level3Open: boolean; level4Open: boolean; level5Open: boolean; level6Open: boolean; level7Open: boolean; level8Open: boolean; randomOpen: boolean; onBack: () => void; onSelect: (level: Level, set: string, random?: boolean, mode?: Mode) => void }) {
  ...
  const [step, setStep] = useState<'level' | 'range'>('level')
...
export function questionPool(choice: Choice): Kana[] {
  return choice.level === 1 ? kanaGroups[choice.set]
    : choice.level === 2 ? pairKana(choice.set)
      : choice.level === 3 ? allKana
        : choice.level === 4 ? dakutenGroups[choice.set]
          : choice.level === 5 ? youonGroups[choice.set]
            : choice.level === 6 ? voicedYouonGroups[choice.set]
              : choice.level === 7 ? allAvailableKana
                : shortWords
}

function Game({ choice, back, finish }: { choice: Choice; back: () => void; finish: (result: GameResult) => void }) {
  const questions = useMemo(() => { const source = questionPool(choice); const selected = choice.level === 7 ? shuffle(source).slice(0, 30) : choice.level === 8 ? shuffle(source).slice(0, 10) : source; return choice.random ? shuffle(selected) : selected }, [choice])
  ...
  const options = useMemo(() => shuffle([q.roma, ...shuffle(questionPool(choice).filter(x => x.roma !== q.roma)).slice(0, Math.min(4, questionPool(choice).length - 1)).map(x => x.roma)]), [q, choice])
...
function Record({ progress, initialLevel, onBack, reset }: { progress: Progress; initialLevel: Level; onBack: () => void; reset: () => void }) {
  ...
  const [compactLandscape, setCompactLandscape] = useState(() => typeof window !== 'undefined' && window.matchMedia('(max-height: 520px) and (orientation: landscape)').matches)
  const [page, setPage] = useState(0)
  useEffect(() => { const query = window.matchMedia('(max-height: 520px) and (orientation: landscape)'); const update = () => setCompactLandscape(query.matches); update(); query.addEventListener('change', update); return () => query.removeEventListener('change', update) }, [])
  ...
  const pageCount = Math.ceil(stages.length / 5)
  const visibleStages = compactLandscape ? stages.slice(page * 5, page * 5 + 5) : stages
  return <section className="content-page record-page"><button className="back-button" onClick={onBack}>← もどる</button><div className="page-heading record-heading"><h2>きろく</h2><p>クリアした ステージに いろが つくよ。</p></div><div className="record-level-tabs" aria-label="レベルを えらぶ">{([1, 2, 3, 4, 5, 6, 7, 8] as Level[]).map(value => <button aria-label={`レベル${value}`} aria-pressed={level === value} className={level === value ? 'selected' : ''} onClick={() => { setLevel(value); setPage(0) }} key={value}><span className="level-tab-full">レベル{value}</span><span className="level-tab-short" aria-hidden="true">{value}</span></button>)}</div><div className={`record-stage-grid record-level-${level} ${compactLandscape ? 'compact-stage-page' : ''}`}>{visibleStages.map(({ label, choice }) => { const status = stageStatus(progress, choice); return <div className={`record-stage ${status.completed ? 'completed' : ''}`} key={id(choice)}><strong>{label}</strong><StatusMarks status={status} showEmpty showSpeed={level < 3} />{level === 3 && <small className="record-time">{status.bestTime === undefined ? 'さいたん －' : `さいたん ${status.bestTime.toFixed(1)}びょう`}</small>}</div> })}</div>{compactLandscape && pageCount > 1 && <div className="record-page-controls"><button disabled={page === 0} onClick={() => setPage(value => value - 1)}>← まえの5こ</button><span>{page + 1} / {pageCount}</span><button disabled={page === pageCount - 1} onClick={() => setPage(value => value + 1)}>つぎの5こ →</button></div>}<button className="reset-button" onClick={reset}>きろくを リセット</button></section>
}
